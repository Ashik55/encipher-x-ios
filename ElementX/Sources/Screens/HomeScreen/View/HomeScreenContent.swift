...
struct HomeScreenContent: View {
    ...
    private var roomList: some View {
        GeometryReader { geometry in
                ...
                case .skeletons:
                    LazyVStack(spacing: 0) {
                        ForEach(context.viewState.visibleRooms) { room in
                            HomeScreenRoomCell(
                                room: room, context: context, isSelected: false
                            )
            ...
            .onChange(of: context.isSearchFieldFocused) { isFocused in
            ...
            .onChange(of: context.viewState.visibleRooms) {
                ...
                DispatchQueue.main.async {
                    guard !scrollViewAdapter.isScrolling.value,
            ...
            .background {
                Button("") {
                    print("🔍 Global search button clicked")
            ...
            .scrollBounceBehavior(
                context.viewState.roomListMode == .empty
                    ? .basedOnSize : .automatic
            )
    ...
    private var topSection: some View {
            ...
            || context.viewState.slidingSyncMigrationBannerMode == .show
        {
            VStack(spacing: 0) {
                ...
                } else if case let .show(state) = context.viewState
                    .securityBannerMode,
    ...
    private func delayedUpdateVisibleRange() {
        ...
        let adjustedContentSize = max(
            scrollView.contentSize.height - scrollView.contentInset.top
                - scrollView.contentInset.bottom, scrollView.bounds.height)
        let cellHeight =
            adjustedContentSize / Double(context.viewState.visibleRooms.count)

        let firstIndex = Int(
            max(0.0, scrollView.contentOffset.y + scrollView.contentInset.top)
                / cellHeight)
        let lastIndex = Int(
            max(0.0, scrollView.contentOffset.y + scrollView.bounds.height)
                / cellHeight)

        // This will be deduped and throttled on the view model layer
        context.send(
            viewAction: .updateVisibleItemRange(firstIndex..<lastIndex))
    }
}
