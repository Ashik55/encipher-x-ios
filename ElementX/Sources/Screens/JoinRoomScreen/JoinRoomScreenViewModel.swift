...
class JoinRoomScreenViewModel: JoinRoomScreenViewModelType,
    JoinRoomScreenViewModelProtocol
{
    ...
    private let actionsSubject:
        PassthroughSubject<JoinRoomScreenViewModelAction, Never> = .init()
    ...
    private func loadRoomDetails() async {
        ...
        case .failure:
            userIndicatorController.submitIndicator(
                UserIndicator(title: L10n.errorUnknown))
        }
    ...
    private func updateRoomDetails() async {
        ...
        // Calculate avatar logic here
        let avatar: RoomAvatar
        if let roomAvatar = info?.avatar,
            case let .room(_, _, avatarURL?, _) = roomAvatar
        {
            // avatarURL is already unwrapped and guaranteed to be non-nil
            avatar = roomAvatar
        } else if let inviter = inviter {
            avatar = .room(
                id: roomID,
        ...
        state.roomDetails = JoinRoomScreenRoomDetails(
            name: info?.displayName ?? inviter?.displayName,
            topic: info?.topic,
            canonicalAlias: info?.canonicalAlias,
            avatar: avatar,  // Use the calculated avatar
    ...
    private func updateMode() async {
        ...
        print("RoomDetails==> \(state.roomDetails)")
        print("RoomDetails Avatar ==>\(state.roomDetails?.avatar)")
        ...
        if let roomPreview {
            ...
            case .banned:
                state.mode = .banned(
                    ...
                    reason: membershipDetails?.ownRoomMember
            ...
            default:
                ...
                case .knock, .knockRestricted:
                    state.mode =
                        appSettings.knockingEnabled ? .knockable : .joinable
                case .restricted:
    ...
    private func joinRoom() async {
        ...
        if let alias = state.roomDetails?.canonicalAlias {
            ...
            case .failure(let error):
                MXLog.error(
                    "Failed joining room alias: \(alias) with error: \(error)")
                userIndicatorController.submitIndicator(
                    .init(title: L10n.errorUnknown))
            }
        } else {
            ...
            case .failure(let error):
                MXLog.error(
                    "Failed joining room id: \(roomID) with error: \(error)")
                userIndicatorController.submitIndicator(
                    .init(title: L10n.errorUnknown))
            }
        }
    ...
    private func knockRoom() async {
        ...
        if let alias = state.roomDetails?.canonicalAlias {
            switch await clientProxy.knockRoomAlias(
                alias,
                message: state.bindings.knockMessage.isBlank
                    ? nil : state.bindings.knockMessage)
            {
            case .success:
            ...
            case .failure(let error):
                MXLog.error(
                    "Failed knocking room alias: \(alias) with error: \(error)")
                userIndicatorController.submitIndicator(
                    .init(title: L10n.errorUnknown))
            }
        } else {
            ...
            case .failure(let error):
                MXLog.error(
                    "Failed knocking room id: \(roomID) with error: \(error)")
                userIndicatorController.submitIndicator(
                    .init(title: L10n.errorUnknown))
            }
        }
    ...
    private func showDeclineInviteConfirmationAlert() {
        ...
        state.bindings.alertInfo = .init(
            ...
            primaryButton: .init(
                title: L10n.actionCancel, role: .cancel, action: nil),
            secondaryButton: .init(
                title: L10n.actionDecline, role: .destructive
    ...
    private func showCancelKnockConfirmationAlert() {
        state.bindings.alertInfo = .init(
            ...
            primaryButton: .init(
                title: L10n.actionNo, role: .cancel, action: nil),
            secondaryButton: .init(
                title: L10n.screenJoinRoomCancelKnockAlertConfirmation,
                role: .destructive
    ...
    private func declineInvite() async {
        ...
        userIndicatorController.submitIndicator(
            UserIndicator(
                id: roomID, type: .modal, title: L10n.commonLoading,
                persistent: true))
    ...
    private static let loadingIndicatorIdentifier =
        "\(JoinRoomScreenViewModel.self)-Loading"
    ...
    private func hideLoadingIndicator() {
        userIndicatorController.retractIndicatorWithId(
